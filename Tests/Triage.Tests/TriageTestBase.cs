...
namespace Triage.Tests
{
    public abstract class TriageTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<TriageDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<TriageDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TriageDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TriageDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TriageDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TriageDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<TriageDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TriageDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TriageDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TriageDbContext, Task<T>> func)
        {
            T result;
